...
auto async_action::build_graph(exec::task_graph::node& parent) -> exec::task_graph::node&
{
    ...
    auto* const then_child{ _then_action ? &_then_action->build_graph(self_node) : nullptr };
    auto* const else_child{ _else_action ? &_else_action->build_graph(self_node) : nullptr };
