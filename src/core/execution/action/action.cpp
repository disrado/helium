...
action::action(delegate<bool(const context&)> definition)
    : _definition{
...
action::action(delegate<bool(const context&, std::stop_token)> definition)
    : _definition{ std::move(definition) }
{
}


auto action::execute(exec::task_node& self_node, std::stop_token token) -> result
{
...
{
    self_node.request = exec::sync_task_request{
        .definition{ exec::task_definition{
            [self{ std::static_pointer_cast<action>(shared_from_this()) }, &self_node] (std::stop_token token) -> exec::task_result
            {
